...
interface NavItem {
  ...
  children?: NavItem[]
  /**
   * When true, the item is rendered as an external `<a>` link instead of a
   * `<router-link>`. The `path` is used as the `href`.
   */
  external?: boolean
  /**
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/user-manual/', label: t('nav.userManual'), icon: BookIcon, external: true },
